...
function wSH.checkPerm(perm)
    ...
    if user_id then
        ...
        if nplayer then
            TriggerClientEvent("Notify",source,"aviso","Sistema em uso.",8000)
            return false            
        else
            if vRP.hasPermission(user_id,perm) or vRP.hasPermission(user_id,'developer.permissao') then
                return true
            end
            return false
        end
...
function wSH.new_Wash(value,org)
    ...
    local user_id = vRP.getUserId(source)
    if user_id then
        if vRP.getInventoryItemAmount(user_id,'dirty_money') >= parseInt(value) then
            if vRP.tryGetInventoryItem(user_id,'dirty_money',parseInt(value),true) then
